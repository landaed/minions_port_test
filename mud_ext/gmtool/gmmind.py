...
from mud.gamesettings import GMSERVERIP,GMSERVERPORT
...
class GMConnection(pb.Root):
    ...
    def connect(self, username, password, role):
        ...
        reactor.connectTCP(GMSERVERIP,GMSERVERPORT,self.factory)
        
        x = self.factory.login(UsernamePassword("%s-%s"%(username,role), self.password),self).addCallbacks(self.connected, self.connectionError)
    ...
    def connectionError(self, err):
        LogText("Failed to connect: %s" % err.value)
        pass
