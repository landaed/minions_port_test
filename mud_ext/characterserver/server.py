...
class WorldConnection(pb.Root):
    ...
    def remote_checkCharacterName(self, cname):
        print("####Checking character name", cname)
    ...
    def connected(self,perspective):
        print('### Connected to world: %s' % self.worldName)
    ...
    def failure(self,reason):
        print('### Failed to connect to world: %s, Reason: %s' % (self.worldName, reason))
        del WORLD_CONNECTIONS[self.worldName]
    ...
    def remote_contestLevelUpEvent(self,publicName,levelType,Level):
        ...
        if MASTERIP == "test.prairiegames.com":
            return 0
    ...
    def remote_checkGrants(self, publicName):
        # TODO: how do grants work? they're not in the TMMOKit... I guess they're given by GMs?
        print("Grants aren't support...")

        # it looks like this is how they work... so we can get these from the character server DB... will have to create a table

        # [(id, type, number, units),]
        # type = 'money', 'text', 'item', 'monster'
        # if type == 'money': number = platinum, units = 'platinum'
        # if type == 'text': units = message
        # if type == 'item': units = itemname (in quotes?)...
        # if type == 'monster': units = monster name

        return None

    def romote_confirmGrants(self, publicName, grantIds):
        # TODO: I guess this marks them as confirmed? so we'd need to transfer them to the character
        # I think items are handled like loot, would need to check how they appear
        return
...
class MasterConnection(pb.Root):
    ...
    def remote_installPlayer(self, publicName, worldName, premium, subnet):
        ...
        if worldName not in WORLD_CONNECTIONS:
            return (False,"Please select another world server.")
