    ...
    def checkTrainClass(self, player):
        ...
        if not spawn.slevel:
            if spawn.plevel < RPG_MULTICLASS_SECONDARY_LEVEL_REQUIREMENT:
                ...
                player.sendGameText(RPG_MSG_GAME_DENIED,"\\nThe Solinia Online: Premium Edition is required to add a tertiary multiclass.  Please see www.prairiegames.com for more information.\\n\\n")
                return False
...
#                    mob.health = int(self.healthMod * float(mob.maxHealth))
    ...
    def doOpenMail(self,player):
        if (self.openMail):
            player.mind.callRemote("openMail")            
