...
def CmdInteract(mob, args):
    ...
    if not (spawn.dialog or vendor or spawn.flags & RPG_SPAWN_BANKER or spawn.flags & RPG_SPAWN_AUCTIONEER):
        return
    ...
    # Check if the target is an auctioneer.
    if spawn.flags & RPG_SPAWN_AUCTIONEER:
        player.mind.callRemote("openAuction",spawn.auctioneer)
        player.auctioneer = spawn.auctioneer
        return
...
def CmdVersion(mob,args):
    msg = "Solinia Online World Server (v)1.01a\\n"
    msg +="Solinia Online Database (v)1.0a\\n"
    mob.player.sendGameText(RPG_MSG_GAME_GLOBAL,msg)
