            ...
            if code == V_STAT:
                if item.spellEnhanceLevel == 9999:  # indicates an enchanted item, additional stat value
                    for var in variants:
                        ...
                        dbconn.execute("insert into ItemVariant values(NULL, %d, '%s', %d, %d, %d, %d, %d, %d);"%(code,var[0],intValue,maxItemList,floatHack,0,0,0))
                else:
                    for var in variants:
                        dbconn.execute("insert into ItemVariant values(NULL, %d, '%s', %d, %d, %d, %d, %d, %d);"%(code,var[0],int(var[1]),maxItemList,0,0,0,0))
            elif code == V_WEAPON:
                dbconn.execute("insert into ItemVariant values(NULL, %d, '%s', %d, %d, %d, %d, %d, %d);"%(code,'',int(variants[0]),maxItemList,int(variants[1]),int(variants[2]),int(variants[3]),0))
            elif code == V_BANEWEAPON:
                dbconn.execute("insert into ItemVariant values(NULL, %d, '%s', %d, %d, %d, %d, %d, %d);"%(code,variants[0],int(variants[1]),maxItemList,0,0,0,0))                                                              
