...
class Character(Persistent):
    ...
    ## @brief (Persistent Integer) Unique ID for the Auction.  Stored in the AH database
    auctionIDN = IntCol(default = 0)    
    ...
    def gainXP(self, amount, clamp=True, rez=None, clampAdjust=0):
        ...
        if RPG_BUILD_DEMO and not self.player.premium:
            ...
            if spawn.plevel >= RPG_DEMO_PLEVEL_LIMIT and pgain:
                ...
                msg = True
                self.player.sendGameText(RPG_MSG_GAME_DENIED,"\\n%s has reached primary level %i and can gain more experience by training in a secondary class or purchasing the Solinia Online: Premium Edition.  The Premium Edition will also allow %s to use premium gear and multiclass in a third class to level 100!  Please see www.prairiegames.com for more information.\\n\\n"%(self.spawn.name, RPG_DEMO_PLEVEL_LIMIT, self.spawn.name))
            ...
            if spawn.slevel >= RPG_DEMO_SLEVEL_LIMIT and sgain:
                ...
                # has been reached.
                if not msg:
                    self.player.sendGameText(RPG_MSG_GAME_DENIED,"\\n%s has reached secondary level %i and can gain more experience by purchasing the Solinia Online: Premium Edition.  The Premium Edition will also allow %s to use premium gear and multiclass in a third class to level 100!  Please see www.prairiegames.com for more information.\\n\\n"%(self.spawn.name, RPG_DEMO_SLEVEL_LIMIT, self.spawn.name))
                        ...
                        if not item.isUseable(self.mob):
                            ...
                            if RPG_BUILD_DEMO and not self.player.premium:
                                if item.level>=50 or item.itemProto.flags&RPG_ITEM_PREMIUM:
                                    self.player.sendGameText(RPG_MSG_GAME_DENIED,"\\nThis item requires the Solinia Online: Premium Edition.  Please see www.prairiegames.com for more information.\\n\\n")
