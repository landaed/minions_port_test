...
def Damage(mob, inflictor, amount, dmgType, textDesc=None, doThorns=True, \
    ...
    # If the mob can take damage we want to put this timer on so they will chase outside of their follow radius. 
    mob.damageTimer = durSecond*10 
    ...
    #Floating Text
    msg = str( int(amount) )      # just print a number for now
    color = (0,255,0)      # Green
    color2 = (255,0,0)     # Red
    if inflictor and inflictor.player and inflictor.player.mind:
        inflictor.zone.simAvatar.mind.callRemote("showDamage", mob.simObject.id, msg, color )
    if mob and mob.player and mob.player.mind and mob.attacking:
        mob.zone.simAvatar.mind.callRemote("showDamage", mob.simObject.id, msg, color2 )
