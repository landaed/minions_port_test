...
from mud.world.item import ItemInstance,Item,ItemInstanceReceive,MailItemCache
...
class PlayerAvatar(Avatar):
        ...
        if RPG_BUILD_DEMO and not self.player.premium:
            ...
            for c in self.player.characters:
                if c.spawn.realm != RPG_REALM_MONSTER and c.spawn.name not in names:
                    nc+=1
            if nc>=3:
                return (-1,"You may have 3 characters in the Solinia Online Free Version.  The Premium Version allows up to 24 characters.  Please see www.prairiegames.com for ordering information.",None)
    ...
    def perspective_newMonster(self,mname,mspawn):
        ...
        if RPG_BUILD_DEMO and not self.player.premium:
            ...
            for c in self.player.characters:
                if c.spawn.realm == RPG_REALM_MONSTER and c.spawn.name not in names:
                    nc+=1
            if nc >=1:
                return (-1,"You may have 1 monster in the Solinia Online Free Version.  The Premium Version allows up to 10 monsters.  Please see www.prairiegames.com for ordering information.",None)
        ...
        if RPG_BUILD_DEMO and not self.player.premium:
            ...
            if src.plevel > 20:
                return (-1,"You may create monsters greater than level 20 with the Solinia Online Premium Edition.  Please see www.prairiegames.com for ordering information.",None)
    ...
    #Beginning of Auction House Calls
    ...
    def sendFail(self):
        self.mind.callRemote("sendAHFail")
        return
        
    #Call back if ItemInfo is successfully obtained from the AH Server.  Passes it back to the client
    def AHItemInfoSuccess(self, results):
        results.setProto(results.auctionItemID)
        self.mind.callRemote("setItemInfo", results.itemInfo)
    ...
    #type 1 - Search 2 - Selling List
    def AHRefreshSuccess(self, updatedDB, type):
        from datetime import datetime
        self.player.auctionLast = datetime.now()
        self.mind.callRemote("updateAuctionDB", updatedDB, type)
            
    #Call from the client to get a refreshed DB.  Fail if time is not up and continue if so
    #RPG_AUCTION_SEARCH_REFRESH - Stored in defines.py and is set to 1 minute.  Referenced by both server and client
    #type 1 - Search 2 - Selling List
    def perspective_refreshAuctionList(self, type):
        from time import time,mktime
        if self.player.auctioneer <= 0:
            return
            
        #Bastard conversion to use time() to check instead of datetime
        lastCheck = mktime(self.player.auctionLast.timetuple())+1e-6*self.player.auctionLast.microsecond
        if (lastCheck + RPG_AUCTION_SEARCH_REFRESH < time()):
            d = self.world.AHPerspective.callRemote("AHAvatar","refreshAuctionList",self.player.auctioneer)
            d.addCallback(self.AHRefreshSuccess,type)
            d.addErrback(self.AHFailure)
        else:
            self.mind.callRemote("updatelocalDBTime", lastCheck, type)
            
    #Updates the character's auctionID
    def AHAuctionIDSuccess(self, id):
    ...
    #If failed spit out an error      
    def AHAuctionRevokeSuccess(self,values):
        auctionID = values[0]
        itemName = values[1]
        ...
        self.player.sendGameText(RPG_MSG_GAME_LOOT, "You have successfully revoked %s.\\n"%itemName)
        self.mind.callRemote("updatelocalDBPurchase", auctionID)            
                
    #Revokes an item a player sold...check to make sure no one has a bid on it first though
    def perspective_AHRevokeSell(self,auctionID):
        self.world.AHPerspective.callRemote("AHAvatar","checkRevoke",auctionID,self.player.curChar.name).addCallbacks(self.AHAuctionRevokeSuccess, self.AHFailure)
            
    #Call Back if AH selling process was a success.  Do some more checking to make sure no one is trying to be fancy and cheat
    #Ends with an update to the client to add the item to the local DB to avoid full refresh
    def AHSellSuccess(self, success,slot,oitem,char,fare,bidTin,buyTin,duration):
        itemfound = 0
        auctionID = success[0]
        charID = success[1]
        ...
        #All good, take the money and the item and let the client know things are good!
        protoID = item.itemProto.id
        self.player.takeMoney(fare)
        self.player.sendGameText(RPG_MSG_GAME_LOOT, "You have successfully placed %s up for auction.\\n"%item.name)
        self.player.takeItem(item)
        self.mind.callRemote("clearAHWindow")
        self.mind.callRemote("updatelocalDBSell", buyTin, bidTin, auctionID, duration, charID,protoID)
        return
        
    #Call Back If AH Buyout was completely successful in the 2nd phase it calls a local DB update on the client to remove the auction Item
    def finishAHBuyoutSuccess(self, values):
        auctionID = values[0]
        seller = values[1]
        itemName = values[2]
        if auctionID > 0:
            self.player.sendGameText(RPG_MSG_GAME_LOOT, "You have successfully purchased %s.\\n"%itemName)
            self.mind.callRemote("updatelocalDBPurchase", auctionID)
        try:
            char = Character.byName(seller)
            if char and char.player:
                char.player.sendGameText(RPG_MSG_GAME_LOOT, "You have successfully sold %s.\\n"%itemName)
        except:
            pass
            
    #Call back if buyout first phase was a success.  Check the player for the cost and if all is well send the item and update the DB
    def AHBuyoutSuccess(self, values):
        cost = values[0]
        auctionID = values[1]
        iid = values[2]
        seller = values[3]
        ...
        if not self.player.checkMoney(cost):
            #Reset the buyout flag since this item is in play now.
            self.world.AHPerspective.callRemote("AHAvatar","revokeAHBuyout",auctionID)
            self.sendFail()
            return
  
        #Take the player's money and send the item over
        self.player.takeMoney(cost)
        char = self.player.curChar

        #Finish up the buyout process by removing it from the DB
        self.world.AHPerspective.callRemote("AHAvatar","finishAHBuyout",cost,auctionID,iid,char.name,seller,bidder_id,bid_cost).addCallbacks(self.finishAHBuyoutSuccess, self.AHFailure)         

    #Initial request from the client to purchase an item
    def perspective_AHBuyout(self,auctionID):    
        if self.player.curChar.auctionIDN <= 0 or self.player.auctioneer <= 0:
            self.sendFail()
            return
                    
        tin = ExpandMoney(self.player.tin, self.player.copper, self.player.silver, self.player.gold, self.player.platinum)
        self.world.AHPerspective.callRemote("AHAvatar","buyout",auctionID,tin,self.player.curChar.auctionIDN,self.player.auctioneer).addCallbacks(self.AHBuyoutSuccess, self.AHFailure)
        
    #Call Back If AH Bid was completely successful.  Need to send out a few updates now
    def finishAHBidSuccess(self, values):
        auctionID = values[0]
        last_bidder = values[1]
        itemName = values[2]
        cost = values[3]
        if auctionID > 0:
            self.player.sendGameText(RPG_MSG_GAME_LOOT, "You have successfully bid on %s.\\n"%itemName)
            self.mind.callRemote("updatelocalDBBid", auctionID,cost)
        try:
            char = Character.byName(last_bidder)
            if char and char.player:
                char.player.sendGameText(RPG_MSG_GAME_LOOT, "You have lost your bid for %s.\\n"%itemName)
        except:
            pass
        
    #Call back if buyout first phase was a success.  Check the player for the cost and if all is well send the item and update the DB
    def AHBidSuccess(self, values):
        cost = values[0]
        auctionID = values[1]
        origcost = values[2]
        ...
        #Update the DB with the Bid information
        self.world.AHPerspective.callRemote("AHAvatar","finishAHBid",cost,auctionID,char.auctionIDN,origcost,char.name).addCallbacks(self.finishAHBidSuccess, self.AHFailure)
        
    #Initial request from the client to bid on an item
    def perspective_AHBid(self,auctionID):
        if self.player.curChar.auctionIDN <= 0 or self.player.auctioneer <= 0:
            self.sendFail()
            return
            
        tin = ExpandMoney(self.player.tin, self.player.copper, self.player.silver, self.player.gold, self.player.platinum)
        self.world.AHPerspective.callRemote("AHAvatar","bid",auctionID,tin,self.player.curChar.auctionIDN,self.player.auctioneer).addCallbacks(self.AHBidSuccess, self.AHFailure)
        
    #Initial request to sell an item.  A lot of checks here to make sure the client validation was proper    
    def perspective_AHSell(self,cid,protoID,slot,bidTin,buyTin,duration,fare):
        ...
        #Check fare.  Recalculate here on the server
        if buyTin:
            startCost = buyTin
        else:
            startCost = bidTin
        ...
        #Prep the ghosted item to be an ItemInstance (Receive is the wrapper to send it between the AH and World Server)
        aitem = ItemInstanceReceive(item.item,True)
        if not item.item: #Not stored on player...blarg!
            aitem.loadFromAltItem(item)
        #Send all this crap to the server
        if not self.player.world.singlePlayer: 
             d = self.world.AHPerspective.callRemote("AHAvatar","placeForSale",aitem,bidTin,buyTin,char.name,item.itemProto.id,duration,slot,self.player.auctioneer) 
             d.addCallback(self.AHSellSuccess,slot,item,char,fare,bidTin,buyTin,duration) 
             d.addErrback(self.AHFailure)
        
    def cleanCharacterAuctions(self,cname): 
      if not self.player.world.singlePlayer: 
            self.world.AHPerspective.callRemote("AHAvatar","deleteCharacter",cname)
        
    #End of Auction House Calls
    ...
    #Send Generic fail message
    def sendMailFail(self):
        self.mind.callRemote("sendMailFail")
        return
        
    #Delete mail from cache and then Mail Server
    ...
    def refreshInboxSuccess(self,mailList, page):
        from datetime import datetime
        ...
        if len(deleteList):
            ...
            self.player.sendGameText(RPG_MSG_GAME_LOOT, "Some mail has expired!\\n")
        ...
        for id, mail in mailList.items():
            ...
            mailCache = MailItemCache(mail)
            if id > self.player.lastMailID:
                self.player.lastMailID = id
            self.player.mailList[id] = mailCache
            toclient[id] = mailCache
            
        if (len(toclient)):
            if prevMailID:
                self.player.sendGameText(RPG_MSG_GAME_LOOT, "There is new mail for you to read!\\n")
    ...
    def perspective_refreshInboxList(self,page):
        ...
        d.addErrback(self.MailFailure)
            
    def finishsendMail(self, auctionID,slot,tin,oitem):
        char = self.player.curChar
        #Item failed to add to the DB...not good
        if not auctionID:
        ...
        self.player.takeMoney(tin)
        self.player.sendGameText(RPG_MSG_GAME_LOOT, "The piece of mail has been delivered.\\n")
    ...
    def perspective_sendMail(self, mail,protoID, slot):
        ...
        d = self.world.MailServerPerspective.callRemote("MailAvatar","sendMail",mail,protoID)
    ...
    def perspective_deleteCharacter(self,cname):
        ...
        if not AVATAR:
            ...
            char.destroySelf()
            self.cleanCharacterAuctions(cname)
            self.cleanCharacterMails(cname)
        ...
        if len(entities) <= 1:
            ...
            for other_mob in zone.activeMobs:
                ...
                if dist < closest_dist:
                    closest_dist = dist
                    closest_name = getattr(other_mob, 'name', '?')
                if dist <= fallback_range:
            ...
            if entities:
                entities[0]["_debug"] = "canSee=%d activeMobs=%d nearest=%s(%.0f) found=%d pos=%s" % (
                    len(visible_ids), len(zone.activeMobs),
                    closest_name or "none", closest_dist,
                    len(entities) - 1,
