...
class ItemContainer:
    ...
    # Checks to see if the container has contents or not.
    def hasContent(self):
        if len(self.content):
            return True
        else:
...
class ItemInstance:
    def __init__(self, item=None, auction=False):
        self.worthIncreaseTin  = 0
        # timer for poisons and spell enchantments
        self.procs    = {}
        self.auctionID         = 0 #Used in the auction info only
        self.auctionItemID     = 0 #Used in the auction info only
        if not auction:
    ...
    def loadFromAltItem(self, item):
        ...
        self.variants          = copy(item.variants)            
    ...
    #Called by the server to set the Proto on an auction item since it is not set during the init process.
    def setProto(self, id):
        self.itemInfo = ItemInfo(self)
        icon = ItemProto._connection.getConnection()
        proto = ItemProto.get(icon.execute("SELECT id FROM item_proto WHERE id = %d LIMIT 1;"%id).fetchone()[0])
        self.itemProto = proto
        #self.name = proto.name
        if self.bitmap == "":
            self.bitmap = proto.bitmap
        self.refreshFromProto()
        
    #sets item values for AH and Mail Server
    def setItemAH(self,conn,id):
        from mud.world.itemvariants import ItemVariantsAHLoad
        stack,use,quality,food,drink,repairmax,repair,lifetime,desc,level,spell,bitmap,variants,crafted,name = conn.execute("SELECT stackCount,useCharges,quality,food,drink,repairMax,repair,lifeTime,descOverride,levelOverride,spellenhanceLevel,bitmap,hasVariants,crafted,name FROM ItemInstance WHERE item_list_id = %s;"%id).fetchone()
        self.stackCount        = stack
        self.useCharges        = use
        self.quality           = quality
        self.food              = food
        self.drink             = drink
        self.repairMax         = repairmax
        self.repair            = repair
        self.lifeTime          = lifetime
        self.descOverride      = desc
        self.levelOverride     = level
        self.spellEnhanceLevel = spell
        self.bitmap            = bitmap
        self.hasVariants       = variants
        self.crafted           = crafted
        self.name              = name
        ItemVariantsAHLoad(self, conn, id)
        return
    ...
    def clone(self):
        ...
        nitem.auctionID         = self.auctionID
    ...
    def refreshFromProto(self,forPet = False):
        ...
        #Could fail in auction items....so let it fail
        try:
            self.itemInfo.reset()
        except:
            pass
...
#Wrapper for the ItemInstance Class so PB can send it back and forth AH<-->World<-->Client
class ItemInstanceReceive(ItemInstance,pb.RemoteCopy,pb.Copyable):
    pass
        
pb.setUnjellyableForClass(ItemInstanceReceive, ItemInstanceReceive)  
 
#Wrapper for the Mail Items so PB can send it back and forth Mail Server<-->World<-->Client
class MailItem(pb.RemoteCopy,pb.Copyable):
    def __init__(self, mail_from="", subject="", mail_to="", tin=0, message="", time_end=0, id=0):
...
pb.setUnjellyableForClass(MailItem, MailItem)
 
class MailItemCache(pb.Cacheable):
    ...
    def getStateToCacheAndObserveFor(self, perspective, observer):
        ...
        state['SUBJECT']=mailItem.subject
        state['MAILFROM']=mailItem.mail_from
        state['TIN']=mailItem.tin
        state['MESSAGE']=""
        state['ITEM']=None
        state['TIMEEND']=mailItem.time_end
        if mailItem.item:           
            state['ITEMPROTOID']=mailItem.item.itemProto.id
        else:
            state['ITEMPROTOID']=0
        state['ID']=mailItem.id       
    ...
    def updateDesc(self):
        changed = {}
        mailItem = self.mailItem
        self.state['MESSAGE'] = changed['MESSAGE'] = mailItem.message 
    ...
    def updateItem(self):
        ...
        mailItem = self.mailItem
        if mailItem.item:
            self.state['ITEM'] = changed['ITEM'] = mailItem.item.itemInfo
        for o in self.observers: 
            o.callRemote('updateChanged', changed)   
...
class MailItemCacheGhost(pb.RemoteCache):
...
pb.setUnjellyableForClass(MailItemCache, MailItemCacheGhost)    
