    ...
    def remote_transferPlayer(self,publicname,pbuffer,charname,cbuffer,zonename,cvalues,remoteLeaderName,guildInfo):
        ...
        for cluster,a in ZoneClusterAvatar.avatars.items():
            ...
            if zonename in a.zoneNames:
                ...
                d = avatar.callRemote("transferPlayer",publicname,pbuffer,charname,cbuffer,code,premium,remoteLeaderName,guildInfo)
                ip = None
                if a.imp:
                    ip = a.imp.ip
                wip = ip
                ...
                d.addCallback(self.playerTransfered,wip,wport,zport,zpassword)
                return d
            index+=1
                
        raise Exception("Unknown Zone! %s" % zonename)
