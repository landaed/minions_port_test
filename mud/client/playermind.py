...
class PlayerMind(pb.Root):
    ...
    def __init__(self):
        ...
        global PLAYERMIND
        PLAYERMIND = self
        self.lastChatString = "nonsense string that is long and never possible"
    ...
    def tick(self):
        ...
        try:
            ...
            from gui.auctionGui import AUCTIONGUI
            ...
            BUFFWND.tick()
            AUCTIONGUI.tick() # refreshes coin values
            MAILGUI.tick() # Periodic refresh of mail information and information to the character
    ...
    def camp_main(self, result, quit=False):
        ...
        if self.rootInfo:
            try:
                #Auction and Mail Cached values to purge
                from gui.auctionGui import AUCTIONGUI
                from gui.mailGui import MAILGUI
                from gui.mailInfoGui import MAILINFOGUI
                AUCTIONGUI.cleanup()
                MAILGUI.cleanup()
                MAILINFOGUI.cleanup()
    ...
    def remote_startSelectron(self,ghostid):
        ...
        if int(TGEGetGlobal("$pref::Video::ShowSelectrons")):
            TGECall("processSelectron", ghostid,6)
    ...
    #Start Auction House Calls
    #Calls are from the server to the client. Passes values on to the Auction GUI were needed
    #Generic fail message
    def remote_sendAHFail(self):
        TGECall("MessageBoxOK","Your Auction House Action Failed","Something happened that caused the transaction to fail.  Please try again and contact an imm if you continue to have a problem.")
     
    #Sets Item Information when right clicking/hovering
    def remote_setItemInfo(self, ItemInfo):
        from gui.auctionGui import AUCTIONGUI
        AUCTIONGUI.setItemInfo(ItemInfo)
        
    #Updates the local DB with a purchase by removing it instead of pulling down a new DB
    def remote_updatelocalDBPurchase(self,auctionID):
        from gui.auctionGui import AUCTIONGUI
        AUCTIONGUI.updatelocalDBPurchase(auctionID)
        
    #Updates the local DB with a bid by refreshing the values
    def remote_updatelocalDBBid(self,auctionID,cost):
        from gui.auctionGui import AUCTIONGUI
        AUCTIONGUI.updatelocalDBBid(auctionID,cost)
        
    #Updated DB from the server
    #type 1 - Search 2 - Selling List
    def remote_updateAuctionDB(self, updatedDB, type):
        from gui.auctionGui import AUCTIONGUI
        AUCTIONGUI.updateDB(updatedDB,type)
        
    #Update the DB Time on the client.  Used to sync the server/client if they are off
    #type 1 - Search 2 - Selling List
    def remote_updatelocalDBTime(self, time, type):
        from gui.auctionGui import AUCTIONGUI
        AUCTIONGUI.updateDBTime(time,type)
    
    #Clears the AH window of values    
    def remote_clearAHWindow(self):
        from gui.auctionGui import AUCTIONGUI
        AUCTIONGUI.clearresults()
        
    #Update the local DB with an item placed up on the auction house
    def remote_updatelocalDBSell(self, buyTin, bidTin, auctionID, duration, charID,protoID):
        from gui.auctionGui import AUCTIONGUI
        AUCTIONGUI.updatelocalDBSell(buyTin, bidTin, auctionID, duration, charID,protoID)
    #END OF AH Calls
    
    #START Mail Server Calls
    #Generic fail message
    def remote_sendMailFail(self):
        TGECall("MessageBoxOK","Your Mail Server Action Failed","Something happened that caused the transaction to fail.  Please try again and contact an imm if you continue to have a problem.")
   
    #Sends a list of mail updates to the client
    def remote_refreshInboxList(self, mailList):
        from gui.mailGui import MAILGUI
        MAILGUI.refreshInboxList(mailList)
        
    #Sends ids of mail to be deleted that expired
    ...
    def remote_clearMailWindow(self):
        from gui.mailGui import MAILGUI
        MAILGUI.clearresults()
    ...
    def remote_openAuction(self,realm):
        from gui.auctionGui import AUCTIONGUI
        AUCTIONGUI.setRealm(realm)
        TGEEval("canvas.pushDialog(AuctionGui);")   
         
    def remote_openMail(self):
        TGEEval("canvas.pushDialog(MailGui);")         
...
def PyDoCommand(myargs, insertCurChar=True, indexHack=None):
    ...
    if not text.startswith('/'): 
        if text == PLAYERMIND.lastChatString: 
            #duplicate chat filtered 
            return 
        else: 
            PLAYERMIND.lastChatString = text 
    else: 
        PLAYERMIND.lastChatString = "another wasted string" 

    #receiveGameText(RPG_MSG_GAME_DENIED, "DEBUG: %s \\n"%text) 
