...
class ItemInfoWnd:
    ...
    def tick(self,cinfo):
        ...
        if not found:
            #check auction
            from auctionGui import AUCTIONGUI
            if int(AUCTIONGUI.auctionItemButton.mouseOver):
                found = True
                self.isItem = True
        ...
        if not found:
            ...
            if int(MAILGUI.sendItem.mouseOver):
                found = True
                self.isItem = True
                ghost = MAILGUI.mailItemGhost
        if not found:
            #check mail Info Window
            from mailInfoGui import MAILINFOGUI
            if int(MAILINFOGUI.itemButton.mouseOver):
                found = True
                self.isItem = True
                ghost = MAILINFOGUI.mailItemGhost
    ...
    def setItem(self,ghost,isLoot=False,isCraft=False,isBank=False):
        ...
        if ghost:
            ...
            #For some odd reason it was thinking NAME wasn't a string so I type casted these all - Xerves
            eval = 'ItemInfoWnd_NameText.setText("%s%s");'%(TEXT_BIG_HEADER,str(ghost.NAME))
            TGEEval(eval)
            ...
            if not isLoot and not isCraft and not isBank:
                ...
                eval = 'PartyWnd_INVITEMNAME.setText("%s%s");'%(TEXT_HEADER,str(ghost.NAME))
                TGEEval(eval)
                
                TGEObject("PetWnd_InvItemBitmap").SetBitmap(currentBitmap)
                eval = 'PetWnd_INVITEMNAME.setText("%s%s");'%(TEXT_HEADER,str(ghost.NAME))
                TGEEval(eval)

            #elif isLoot:
            #    TGEObject("LOOT_ITEMPIC").SetBitmap(currentBitmap)
            #    eval = 'LOOT_ITEMTEXT.setText("%s%s");'%(TEXT_HEADER,str(ghost.NAME))
            #    TGEEval(eval)
            elif isCraft:
                TGEObject("CRAFTING_ITEMPIC").SetBitmap(currentBitmap)
                eval = 'CRAFTING_ITEMTEXT.setText("%s%s");'%(TEXT_HEADER,str(ghost.NAME))
                TGEEval(eval)
            elif isBank:
                TGEObject("NPCWnd_InvItemBitmap").SetBitmap(currentBitmap)
                eval = 'NPCWnd_INVITEMNAME.setText("%s%s");'%(TEXT_HEADER,str(ghost.NAME))
                TGEEval(eval)
