...
class InvPane:
    def __init__(self):
        ...
        #Used by the Auction House to choose the last slot picked before moved to the cursor
        self.lastSlot = 0
        self.page = 0
    ...
    def onInvSlot(self,slot):
        ...
        if RPG_SLOT_CARRY_END > slot >= RPG_SLOT_CARRY_BEGIN:
            slot+=self.page*30
            self.lastSlot = slot
            #Reset Auction GUI values since a new item has been picked
            AUCTIONGUI.clearItemButton()
            #Reset Mail GUI values since a new item has been picked
            MAILGUI.clearItemButton()
