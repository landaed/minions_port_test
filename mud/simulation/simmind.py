...
# --- Clients
    ...
    def setPlayerSkins(self,player,mobInfo,spawninfo):
        ...
        if not spawninfo.textureFeet: #only racial have feet
            ...
            player.setSkin(7,"~/data/shapes/character/textures/%s"%spawninfo.textureExtra)
    ...
    def remote_newSpellEffect(self,srcId,effect,interrupt=True):
        src = self.simLookup[srcId]
        if interrupt and src.spellBeingCast != None:
            try:
                TGEObject(src.spellBeingCast).interrupt()
            # Exception if the object was not found, our spell info
            #  is therefore outdated.
            except:
                pass
            src.spellBeingCast = None
        src.spellBeingCast = src.tgeObject.castSpell(effect)
    ...
    def remote_immobilize(self,id):
        tge = TGEObject(id)
        if tge.getClassName()=="rpgAIPlayer":
    ...
    def remote_casting(self,id,casting,interrupt=True):
        so = self.simLookup[id]
        if interrupt and so.spellBeingCast != None:
            try:
                TGEObject(so.spellBeingCast).interrupt()
            # Exception if the object was not found, our spell info
            #  is therefore outdated.
            except:
                pass
            so.spellBeingCast = None
    ...
    #Floating Text    
    def remote_showDamage(self, id, damageStr, color):
        cmd = 'Show3dDamage(%i, \"%s\", %i, %i, %i);' % (id, damageStr, color[0], color[1], color[2])
        TGEEval( cmd )
...
def PyOnPlayerDeleted(args):
    ...
    id = int(args[1])
    player = TGEObject(id)
    if player.getClassName() == "rpgPlayer":
