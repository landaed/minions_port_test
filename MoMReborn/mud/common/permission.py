# Embedded file name: mud\common\permission.pyo
...
class ColumnPermission(Persistent):
    name = StringCol(default='', notNone=True)
    type = StringCol(default='', notNone=True)
...
class TablePermission(Persistent):
    ...
    read = BoolCol(default=False)
    write = BoolCol(default=False)
    insert = BoolCol(default=False, dbName='pinsert')
    kill = BoolCol(default=False, dbName='pkill')
    update = BoolCol(default=False, dbName='pupdate')
...
class Role(Persistent):
    name = StringCol(default='', alternateID=True, notNone=True)
...
class User(Persistent):
    doPasswordHack = True
    name = StringCol(default='', alternateID=True, notNone=True)
    password = StringCol(default='', notNone=True)
    tempPassword = StringCol(default='')
    banLevel = IntCol(default=0)
    lastConnectSubnet = StringCol(default='')
