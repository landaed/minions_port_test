# Embedded file name: mud\simulation\shared\simdata.pyo
...
class SimMobInfo(pb.Cacheable):
    ...
    def refresh(self):
        try:
            ...
            flying = mob.flying
            if mob.simObject:
                if mob.simObject.waterCoverage >= 0.45:
                    move = mob.swimMove
            size = mob.size
            ...
            if not state['RANGEDATTACK'] or not ranged:
                ...
                if primaryWeapon:
                    ...
                    if primaryWeapon.skill and '2H' in primaryWeapon.skill and (not secondaryWeapon or not mob.skillLevels.get('Power Wield')):
                        mount1 = mount1_material = ''
                        mount2 = mount2_material = ''
                        twohanded = True
                else:
