# Embedded file name: mud\client\gui\friendsWnd.pyo
...
class FriendsWnd(object):
    ...
    def update(self, finfo = None, fromServer = False):
        ...
        try:
            ...
            tc.clear()
            online = set()
            fmap = dict(((friend.upper(), friend) for friend in finfo))
            for i in xrange(len(FRIENDS)):
                friendUpper = FRIENDS[i]
                if fmap.has_key(friendUpper):
                    online.add(friendUpper)
                    friend = fmap[friendUpper]
                    matchGuild, wname, zname = finfo[friend]
                    if matchGuild:
                        guildTag = ' (Guild Member)'
                    else:
                        guildTag = ''
                    tc.addRow(i, '%s%s\t%s\t%s' % (friendUpper,
                     guildTag,
                     wname,
                     zname))
                else:
                    tc.addRow(i, '%s\tAway\t' % friendUpper)

            if fromServer:
                for name in online.difference(self.active):
                    receiveGameText(RPG_MSG_GAME_LEVELGAINED, 'Your friend %s has joined the game!\\n' % name)
                    if playsound:
                ...
                for name in self.active.difference(online):
                    receiveGameText(RPG_MSG_GAME_BLUE, 'Your friend %s has left the game!\\n' % name)
                    if playsound:
    ...
    def onAddFriend(self, name = None):
        ...
        if len(PLAYERSETTINGS.friends) >= 64:
            receiveGameText(RPG_MSG_GAME_DENIED, 'You may have up to 64 friends in your list.\\n')
            return
        else:
            if name == None:
            ...
            if not PLAYERSETTINGS.addFriend(name):
                receiveGameText(RPG_MSG_GAME_DENIED, '%s is already in your list.\\n' % name)
                return
    ...
    def onRemoveFriend(self, name = None):
        ...
        if not name:
            ...
            receiveGameText(RPG_MSG_GAME_GAINED, 'You have removed %s from your friend list.\\n' % friend)
        elif PLAYERSETTINGS.removeFriend(name):
            receiveGameText(RPG_MSG_GAME_GAINED, 'You have removed %s from your friend list.\\n' % name)
        else:
            receiveGameText(RPG_MSG_GAME_DENIED, '%s is not in your friend list.\\n' % name)
        self.submitFriendsList()
    ...
    def onClickFriend(self):
        ...
        commandCtrl.setValue('/tell %s ' % FRIENDS[index].capitalize().replace(' ', '_'))
    ...
    def submitFriendsList(self):
        ...
        if not PLAYERMIND or not PLAYERMIND.perspective:
            return
        try:
            PLAYERMIND.perspective.callRemote('PlayerAvatar', 'submitFriends', PLAYERSETTINGS.friends)
        except:
            pass
