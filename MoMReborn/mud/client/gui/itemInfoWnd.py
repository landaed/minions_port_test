# Embedded file name: mud\client\gui\itemInfoWnd.pyo
...
NICESLOTNAMES = {RPG_SLOT_HEAD: 'Head',
...
class ItemInfoWnd():
    ...
    def tick(self, cinfo):
        ...
        if not self.ghost:
            ...
            self.lastBitmap = ''
        elif self.isItem or self.isLoot or self.isCraft:
    ...
    def setSpell(self, ghost):
        ...
        if icon.startswith('SPELLICON_'):
            ...
            self.lastBitmap = ''
        eval = 'ItemInfoWnd_NameText.setText("%s");' % (TEXT_BIG_HEADER + sinfo.NAME)
        TGEEval(eval)
    ...
    def setItem(self, ghost, isLoot = False, isCraft = False, isBank = False):
        ...
        if ghost:
            ...
            if not isLoot and not isCraft:
                self.isItem = True
            text = ' '.join(('\\cp\\c2%s\\co' % ftext for f, ftext in RPG_ITEM_FLAG_TEXT.iteritems() if ghost.FLAGS & f))
