# Embedded file name: mud\client\gui\tomeGui.pyo
...
import re
SPEECHCOLORCODES = {RPG_MSG_SPEECH_WORLD: 'C8B560',
...
GAMECOLORCODES = {RPG_MSG_GAME_GLOBAL: 'FFFFFF',
...
class TomeGui(object):
    ...
    def receiveGameText(self, textCode, text):
        ...
        text = text.replace('\\n', '\n')
        self.gameText += '<linkcolor:%s><color:%s>%s' % (GAMECOLORCODES[textCode], GAMECOLORCODES[textCode], text)
        while len(self.gameText) > 4000:
    ...
    def receiveSpeechText(self, textCode, text):
        ...
        logtext = TGECall('StripMLControlChars', text)
        logtext = logtext.replace('\\n', '\n')
        if textCode not in (RPG_MSG_SPEECH_SYSTEM, RPG_MSG_SPEECH_PLAYERJOINED, RPG_MSG_SPEECH_ERROR):
            logtext = NEWLINE_REPLACER.sub('\n... ', logtext)
        CoreSettings.LOGCHAT.write('%s - %s' % (s, logtext))
        text = text.replace('\\n', '\n')
        if textCode not in (RPG_MSG_SPEECH_SYSTEM, RPG_MSG_SPEECH_PLAYERJOINED, RPG_MSG_SPEECH_ERROR):
            text = NEWLINE_REPLACER.sub('\n... ', text)
        self.speechText += '<linkcolor:%s><color:%s>%s' % (SPEECHCOLORCODES[textCode], SPEECHCOLORCODES[textCode], text)
        while len(self.speechText) > 4000:
    ...
    def onGlobalChannelToggle(self, v = None, fromStore = False):
        ...
        FilterChannel('M', v)
        if v:
            self.receiveGameText(RPG_MSG_GAME_GAINED, 'You are now listening to mom chat.\\n')
        else:
            self.receiveGameText(RPG_MSG_GAME_GAINED, 'You are no longer listening to mom chat.\\n')
        if not fromStore:
    ...
    def onOffTopicChannelToggle(self, v = None, fromStore = False):
        ...
        FilterChannel('O', v)
        if v:
            self.receiveGameText(RPG_MSG_GAME_GAINED, 'You are now listening to off-topic chat.\\n')
        else:
            self.receiveGameText(RPG_MSG_GAME_GAINED, 'You are no longer listening to off-topic chat.\\n')
        if not fromStore:
            from playerSettings import PLAYERSETTINGS
            from mud.client.playermind import TOGGLEABLECHANNELS
    ...
    def onHelpChannelToggle(self, v = None, fromStore = False):
        ...
        FilterChannel('H', v)
        if v:
            self.receiveGameText(RPG_MSG_GAME_GAINED, 'You are now listening to help chat.\\n')
        else:
            self.receiveGameText(RPG_MSG_GAME_GAINED, 'You are no longer listening to help chat.\\n')
        if not fromStore:
            from playerSettings import PLAYERSETTINGS
            from mud.client.playermind import TOGGLEABLECHANNELS
