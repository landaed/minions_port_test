# Embedded file name: mud\client\gui\trackingWnd.pyo
        ...
        try:
            ...
            for id, values in self.tracking.iteritems():
                ...
                if id == self.trackingId:
                    TGEEval('TRACKING_TRACKINGLIST.addRow(%i,"\\c2%s" TAB %i @"m");' % (id, values[0], ceil(values[2])))
                elif values[3] == 1:
                    TGEEval('TRACKING_TRACKINGLIST.addRow(%i,"\\c3%s" TAB %i @"m");' % (id, values[0], ceil(values[2])))
                else:
                    TGEEval('TRACKING_TRACKINGLIST.addRow(%i,"%s" TAB %i @"m");' % (id, values[0], ceil(values[2])))
            ...
            for description, location in poi.iteritems():
                ...
                TGEEval('TRACKING_TRACKINGLIST.addRow(%i,"\\c4%s" TAB %i @"m");' % (id, description, ceil(dist)))
                if id == prevSelected:
                    found = True
                id -= 1
