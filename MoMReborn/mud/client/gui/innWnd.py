# Embedded file name: mud\client\gui\innWnd.pyo
...
class InnWnd(pb.Root):
    ...
    def remote_setInn(self, cList, pList):
        ...
        for i, ci in enumerate(pList):
            ...
            level = ci.levels[0]
            status = ci.status
            TGEEval('INNPARTY_SCROLLLIST.addRow(%i,"%s" TAB "%s (%i)" TAB "%s");' % (i,
             name,
             klass,
             level,
             status))
        ...
        for i, ci in enumerate(cList):
            ...
            level = ci.levels[0]
            status = ci.status
            TGEEval('INNCHARS_SCROLLLIST.addRow(%i,"%s" TAB "%s (%i)" TAB "%s");' % (i,
             name,
             klass,
             level,
             status))
