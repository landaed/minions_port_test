# Embedded file name: mud\client\gui\tradeWnd.pyo
...
class TradeWnd:
    ...
    def setFromTradeInfo(self, tradeInfo):
        ...
        if p0Accepted:
            eval = 'TRADE_P0_NAME.setText("\\c2%s");' % c0name
        else:
            eval = 'TRADE_P0_NAME.setText("\\c1%s");' % c0name
        TGEEval(eval)
        if p1Accepted:
            eval = 'TRADE_P1_NAME.setText("\\c2%s");' % c1name
        else:
            eval = 'TRADE_P1_NAME.setText("\\c1%s");' % c1name
        TGEEval(eval)
        ...
        for slot, butt in self.p0Items.iteritems():
            if not p0Items.has_key(slot):
                butt.SetBitmap('')
                butt.number = -1

        for slot, butt in self.p1Items.iteritems():
            if not p1Items.has_key(slot):
                butt.SetBitmap('')
                butt.number = -1
