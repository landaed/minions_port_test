# Embedded file name: mud\client\gui\advancePane.pyo
...
class AdvancePane:
    ...
    def setFromCharacterInfo(self, cinfo):
        ...
        try:
            ...
            ci = 0
            for myid, name, level, desc, cost, maxRank in con.execute('SELECT id,name,level,desc,cost,max_rank FROM advancement_proto;'):
                exclusions = [ item[0] for item in con.execute('SELECT DISTINCT exclude FROM advancement_exclusion WHERE advancement_proto_id = %i;' % myid) ]
                exclusionsRedux = [ item[0] for item in con.execute('SELECT DISTINCT exclude FROM advancement_exclusion WHERE advancement_proto_id = %i AND exclude NOT IN (SELECT DISTINCT exclude FROM advancement_exclusion WHERE advancement_proto_id IN (SELECT id FROM advancement_proto WHERE name IN (SELECT exclude FROM advancement_exclusion WHERE advancement_proto_id = %i)));' % (myid, myid)) ]
                try:
                    rank = cinfo.ADVANCEMENTS[currentNames.index(name)][1]
                    TGEEval('AdvanceCurrentTextList.addRow(%i,"%s" TAB "%s");' % (ci, name, '(%i/%i)' % (rank, maxRank)))
                    self.currentChoices.append((name,
                     rank,
                     maxRank,
                     desc,
                     exclusionsRedux))
                ...
                if cost > cinfo.ADVANCE or level > cinfo.PLEVEL:
                    continue
                passed = True
                classReqs = con.execute('SELECT classname,level FROM advancement_class WHERE advancement_proto_id = %i;' % myid).fetchall()
                if len(classReqs):
                    passed = False
                    ...
                    if not passed:
                        continue
                raceReqs = con.execute('SELECT racename,level FROM advancement_race WHERE advancement_proto_id = %i;' % myid).fetchall()
                if len(raceReqs):
                    passed = False
                    ...
                    if not passed:
                        continue
                advanceReqs = con.execute('SELECT require,rank FROM advancement_requirement WHERE advancement_proto_id = %i;' % myid).fetchall()
                if len(advanceReqs):
                    ...
                    if not passed:
                        continue
                self.availableChoices.append((name,
                 cost,
                 maxRank,
                 desc,
                 exclusionsRedux))
            ...
            for i in xrange(len(self.availableChoices)):
                ...
                if name in currentExclusions:
                    ...
                    offset += 1
                    continue
                TGEEval('AdvanceAvailableTextList.addRow(%i,"%s" TAB "%s");' % (ai, name, '(%i/%i)' % (choice[1], choice[2])))
                if name in aprev:
                    aprevindex = ai
                ai += 1
