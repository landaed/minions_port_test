# Embedded file name: mud\world\repair.pyo
from core import *
...
def CheckRepairItem(charname, rskill, rinfo, citem):
    ...
    if not citem:
        receiveGameText(RPG_MSG_GAME_DENIED, 'Please place an item in your cursor.\\n')
        return
    ret = GetItemRepairCost(rskill, citem.REPAIR, citem.REPAIRMAX, citem.LEVEL, citem.CLASSES)
    if not ret:
        receiveGameText(RPG_MSG_GAME_DENIED, '%s has insufficient skill to repair <a:Item%s>%s</a>.\\n' % (charname, GetTWikiName(citem.PROTONAME), citem.NAME))
        return
    if ret == 1:
        receiveGameText(RPG_MSG_GAME_DENIED, "<a:Item%s>%s</a> doesn\\'t need repair.\\n" % (GetTWikiName(citem.PROTONAME), citem.NAME))
        return
...
def CheckRepairAll(charname, rskill, rinfo, charItems, output = True):
    ...
    if iskill:
        if output:
            receiveGameText(RPG_MSG_GAME_DENIED, '%s has insufficient skill to repair %i equipped items.\\n' % (charname, iskill))
...
def CheckRepairParty(charname, rskill, rinfo, charinfos):
    ...
    if not numrepairs:
        ...
        return
    if iskill:
        receiveGameText(RPG_MSG_GAME_DENIED, '%s has insufficient skill to repair %i party items.\\n' % (charname, iskill))
...
def RepairItem(player, char):
    ...
    if citem.repair == citem.repairMax:
        player.sendGameText(RPG_MSG_GAME_BLUE, '%s completely repairs <a:Item%s>%s</a> for %i points! (%i/%i)\\n%s in materials consumed.\\n' % (char.name,
         GetTWikiName(citem.itemProto.name),
         citem.name,
         points,
         citem.repair,
        ...
        player.sendGameText(RPG_MSG_GAME_GAINED, '%s repairs <a:Item%s>%s</a> for %i points! (%i/%i)\\n%s in materials consumed.\\n' % (char.name,
         GetTWikiName(citem.itemProto.name),
         citem.name,
         points,
         citem.repair,
