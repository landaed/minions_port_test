# Embedded file name: mud\world\alliance.pyo
import time
...
class Alliance():
    ...
    def rewardKillFaction(self, killer, mob):
        ...
        for kf in mob.spawn.killFactions:
            if kf.percent < 0:
                kfaction = math.ceil(totalFaction / 2.0 * kf.percent)
            else:
                kfaction = math.ceil(totalFaction / 4.0 * kf.percent)
    ...
    def killPenalty(self, killer, mob):
        ...
        for player in self.members:
            if killer == player:
                player.sendGameText(RPG_MSG_GAME_YELLOW, 'You have %s\\n' % msg)
                for character in player.party.members:
                    character.loseXP(1.5 * float(character.spawn.plevel), False)

            else:
                player.sendGameText(RPG_MSG_GAME_YELLOW, '<a:gamelinkcharlink%s>%s</a> has %s\\n' % (skillerName, killerName, msg))
    ...
    def lootMessage(self, sender, lootitem):
        ...
        for m in self.members:
            if sender == m:
                m.sendGameText(RPG_MSG_GAME_YELLOW, 'You have looted: <a:Item%s>%s</a>\\n' % (GetTWikiName(lootitem.itemProto.name), lootitem.name))
            else:
                m.sendGameText(RPG_MSG_GAME_YELLOW, '<a:gamelinkcharlink%s>%s</a> has looted: <a:Item%s>%s</a>\\n' % (ssenderName,
                 senderName,
                 GetTWikiName(lootitem.itemProto.name),
                 lootitem.name))

    def message(self, sender, msg):
        name = sender.charName
        sname = name.replace(' ', '_')
        msg = 'Alliance: <<a:gamelinkcharlink%s>%s</a>> %s\\n' % (sname, name, msg)
        if self.world.daemonPerspective:
    ...
    def kick(self, name):
        if self.remoteLeaderName != self.leader.publicName:
            ...
            if self.world.daemonPerspective:
                a = Alliance.masterAllianceInfo.get(self.remoteLeaderName)
                if a:
    ...
    def join(self, who):
        ...
        if self.countMembers() >= 6:
            ...
            return False
        elif len(who.alliance.members) > 1:
            self.cancelInvite(who)
            return False
        else:
            for inv in self.invites:
