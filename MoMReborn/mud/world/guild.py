# Embedded file name: mud\world\guild.pyo
from defines import *
...
def GuildCreate(mob, args):
    ...
    if not mob.player.premium:
        mob.player.sendGameText(RPG_MSG_GAME_DENIED, 'Premium Account is required to create guilds.\\nPlease see www.prairiegames.com for more information.\\n\\n')
        return
    if len(args) < 1:
        return
    player = mob.player
...
def GuildDisband(mob, args):
    ...
    if not player.premium:
        player.sendGameText(RPG_MSG_GAME_DENIED, 'Premium Account is required to use guilds.\\nPlease see www.prairiegames.com for more information.\\n')
        return
