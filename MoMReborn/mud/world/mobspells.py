# Embedded file name: mud\world\mobspells.pyo
...
def InitMobSpells():
    ...
    for scname, protoid, sclevel in con.execute('SELECT classname,spell_proto_id,level FROM spell_class WHERE (SELECT spell_type FROM spell_proto WHERE id=spell_proto_id)&%i;' % RPG_SPELL_AICAST):
        d = MOB_SPELLS.setdefault(scname, {}).setdefault(sclevel, {'pet': set(),
         'harmful': set(),
         'beneficial': set(),
         'healing': set()})
...
class MobSpells:
    ...
    def __init__(self, mob, master = None):
        ...
        self.beneTimer = random.randint(0, 270)
    ...
    def think(self):
        ...
        if mob.casting:
            return
        elif mob.sleep > 0 or mob.stun > 0:
            return
        elif mob.master and mob.visibility <= 0:
            return
        else:
            ...
            if mob.target and len(self.harmful) and IsKOS(mob, mob.target) and mob.target.simObject.id in mob.simObject.canSee:
                ...
                if self.petSpell and not mob.pet and not mob.petSpawning and not mob.master and mob.petCounter < 2:
                    proto = self.petSpell
                elif len(self.harmfulSpawnSpells) and not random.randint(0, 2):
                ...
                if not proto:
                    ...
                    elif len(self.slow) and mob.target.slow <= 0 and not random.randint(0, 2):
                        if len(self.slow) == 1:
                            proto = self.slow[0]
                        else:
